"""Ingestão incremental da carga verificada ONS (semi-horária) → load_actual horária no Supabase."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ingestão da carga verificada ONS para load_actual."
    )
    parser.add_argument(
        "--inicio",
...
def fetch_verificada(cod_areacarga: str, inicio: date, fim: date) -> list[dict] | None:
    params = {
        ...
        rows.append(
            {
                ...
                "val_cargaglobal": record["val_cargaglobal"],
                "din_atualizacao": pd.to_datetime(record["din_atualizacao"], utc=True),
            }
        )

    df = pd.DataFrame(rows)
    df = df[df["val_cargaglobal"].notna() & (df["val_cargaglobal"] != 0)]
    hourly = (
        df.groupby("hora_ts", as_index=False)
        .agg(
            load_mw=("val_cargaglobal", "mean"),
            din_atualizacao=("din_atualizacao", "max"),
        )
        .sort_values("hora_ts")
    )
    return hourly
...
def upsert_load_actual(
    ...
    sql = """
        INSERT INTO load_actual (subsystem_id, ts, load_mw, din_atualizacao)
        VALUES %s
...
def main() -> None:
    ...
    try:
        ...
        for codigo in SUBSYSTEMS:
            records = fetch_verificada(codigo, inicio, fim)
            if records is None:
                continue

            hourly = aggregate_hourly(records)
            upserted = upsert_load_actual(conn, subsystem_ids[codigo], hourly)
